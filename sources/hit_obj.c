...
int	cy_side(t_object *cy, t_ray r, t_hit_rec *rec)
{
	...
	c = vec_len_square(vec_cross(vec_sub(r.origin, cy->center), cy->norm)) - (cy->diameter / 2) * (cy->diameter / 2);
...
int	hit_sphere(t_object *sp, t_ray r, t_hit_rec *rec)
{
	...
	c = vec_len_square(oc) - (sp->diameter / 2) * (sp->diameter / 2);
	discriminant = half_b * half_b - a * c;
	if (discriminant < 0)
		return (0);
	...
	rec->t = root;
	rec->point = ray_at(r, root);
	rec->normal = vec_mul(vec_sub(rec->point, sp->center), 1 / (sp->diameter / 2));
	set_face_normal(r, rec);
	return (1);
}
